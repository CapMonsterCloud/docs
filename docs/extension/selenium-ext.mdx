...
В этом руководстве показано, как подключить браузерное расширение CapMonster Cloud к Selenium WebDriver с использованием Node.js.

Этот подход полезен, если вы автоматизируете браузерные сценарии через Selenium и хотите использовать возможности расширения вместо прямой работы с API в коде.

Перед началом убедитесь, что у вас уже есть рабочий архив расширения и API-ключ. Если расширение ещё не настроено, сначала откройте инструкцию [Расширение для браузера Chrome](./extension-main.mdx).
