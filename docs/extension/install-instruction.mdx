...
Эта инструкция подходит для ProjectMaker, если вы хотите использовать браузерное расширение CapMonster Cloud внутри проекта и решать капчи через интерфейс браузера.

Если у вас уже есть CRX-файл, используйте способ №1. Если удобнее ставить расширение напрямую из Chrome Web Store, переходите к способу №2.
...
## Как понять, что всё настроено

После установки расширение должно открываться в ProjectMaker, принимать ваш API-ключ и показывать баланс аккаунта. Если этого не происходит, сначала проверьте версию расширения и корректность API-ключа.
