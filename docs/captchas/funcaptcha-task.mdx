...
:::warning **Внимание!**
* Для решения FunCaptcha необходимо использовать **ваш собственный прокси**.

* Если прокси с авторизацией по IP, необходимо добавить адрес **65.21.190.34** в белый список.

* Используйте один и тот же прокси при получении параметров CAPTCHA на сайте и при создании задачи в CapMonster Cloud.
:::
...
Ключ FunCaptcha (значение `public key` или `pk`).<br />

Пример формата: `EX72CCFB-26EX-40E5-91E6-85EX70BE98ED`
...
Поддомен Arkose Labs (значение `surl`). Указывайте в том случае, если он отличается от стандартного: `client-api.arkoselabs.com`<br />
...
// Для браузера и задачи CapMonster Cloud используйте один и тот же прокси
const proxy = {
    proxyType: "http",
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // FunCaptcha решается только с использованием вашего прокси
  const proxy = {
    proxyType: "http",
    proxyAddress: "123.45.67.89",
    proxyPort: 8080,
