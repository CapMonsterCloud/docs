...
description: "Узнайте, как распознать Hunt CAPTCHA с помощью API сервиса CapMonster Cloud."
---
...
| Сценарий | Параметры в `metadata` | Результат |
| --- | --- | --- |
| Генерация X-HD | `apiGetLib` | X-HD |
| Решение через `meta.token` | `apiGetLib`, `data` | Токен решения CAPTCHA |
| Решение через `widgetUrl` | `apiGetLib`, `widgetUrl` | Токен решения CAPTCHA |
...
Если `widgetUrl` недоступен, сначала сгенерируйте X-HD, получите от сайта `meta.token`, а затем создайте задачу с параметром `data` (*см. подробнее в разделе  [Пример решения Hunt CAPTCHA через data](#пример-решения-hunt-captcha-через-data)*).
...
> Примеры автоматического извлечения параметров см. в разделах [Как получить widgetUrl](#как-получить-widgeturl) и [Пример решения Hunt CAPTCHA через data](#пример-решения-hunt-captcha-через-data) .
</span>
<br />
...
<ParamItem title="widgetUrl (внутри metadata)" type="string" />
...
Используйте метод [createTask](/docs/api/methods/create-task/):
...
Используйте метод [getTaskResult](/docs/api/methods/get-task-result/), чтобы получить fingerprint X-HD или решение Hunt капчи:
...
:::warning Важно:
Используйте актуальные HTTP‑заголовки (подробнее о заголовках в [данном разделе](/docs/api/headers/)) и [корректный `User-Agent`](https://capmonster.cloud/api/useragent/actual).
:::
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type1">
  <TabItem value="nodejs" label="JavaScript (в браузере)" className="method-panel">

<details>
<summary>Генерация параметра X-HD</summary>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Для решения HUNT необходимо использовать собственный прокси
  const proxy = {
    proxyType: "http",
...
<details>
<summary>Решение CAPTCHA</summary>
