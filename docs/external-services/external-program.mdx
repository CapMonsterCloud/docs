...
## Выберите сценарий интеграции

- Если программа уже поддерживает CapMonster Cloud, используйте нативное подключение и API-ключ.
- Если программа умеет работать с сервисом, где можно указать домен API, используйте `api.capmonster.cloud`.
- Если программа поддерживает только сторонние captcha-сервисы, перейдите к инструкции с перенаправлением через `hosts`.
