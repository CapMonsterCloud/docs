...
Этот сценарий подходит, если ваш софт уже умеет работать с одним из перечисленных сервисов, а вы хотите протестировать или подключить CapMonster Cloud без полной переписки интеграции.

Если вы настраиваете новый проект с нуля, обычно проще начать с прямого [API](https://docs.capmonster.cloud/ru/docs/category/api) или официальных SDK из раздела [Начало работы](https://docs.capmonster.cloud/ru/docs/getting-start).
...
В процессе установки потребуется указать API-ключ вашего аккаунта, который вы сможете найти в [личном кабинете](https://dash.capmonster.cloud/).
...
Если вы не уверены, подходит ли вам этот способ интеграции, сравните его с инструкцией [Как подключить CapMonster Cloud к нужной программе](https://docs.capmonster.cloud/ru/docs/external-services/external-program).
